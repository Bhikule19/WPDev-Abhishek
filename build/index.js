...
function myNotes() {
  ...
  function updateNote(e) {
    ...
    const options = {
      ...
      // Use body and stringify the payload. This converts the ourUpdatedPost JavaScript object into a JSON string because the body of the request must be a string for JSON data.
      headers: {
        "Content-Type": "application/json",
        // Indicate the content type. you are informing the server that the data being sent in the body of the request is in JSON format. This is necessary for the server to correctly parse and handle the incoming data.
        "X-WP-Nonce": universityData.nonce //AJAX Request: Using fetch for the DELETE request, including the nonce in the headers.
      }
    };
    fetch(url, options).then(response => {
  ...
  function createNote() {
    ...
    fetch(url, options).then(response => {
      ...
      noteHtml.insertAdjacentHTML("afterbegin", ` <li data-id="${data.id}" class="fade-in-calc">
  ...
  function makeNoteEditable(editListItem) {
    const editNoteFields = editListItem.querySelectorAll(".note-title-field, .note-body-field, .update-note");
    const editNoteButn = editListItem.querySelector(".edit-note");
    editNoteButn.innerHTML = '<i class="fa fa-times" aria-hidden="true"></i>Cancel';
  ...
  function makeNoteReadOnly(editListItem) {
    const editNoteFields = editListItem.querySelectorAll(".note-title-field, .note-body-field, .update-note");
    const editNoteButn = editListItem.querySelector(".edit-note");
    editNoteButn.innerHTML = '<i class="fa fa-pencil" aria-hidden="true"></i>Edit';
    editNoteFields.forEach(field => {
...
function initializeSearch() {
  ...
  document.addEventListener("keydown", e => {
    ...
    if ((e.key === "S" || e.key === "s") && !isOverlay && activeElement.tagName !== "INPUT" && activeElement.tagName !== "TEXTAREA") {
      openOverlay();
  ...
  function getResults() {
    ...
    fetch(universityData.root_url + "/wp-json/university/v1/search?term=" + searchValue).then(response => response.json()).then(results => {
      resultsDiv.innerHTML = `
      <div class="row" >
        <div class="one-third" >
          <h2 class="search-overlay__section-title">General Information</h2>
            ${results.generalInfo.length ? '<ul class="link-list min-list">' : "<p>No general Information</p>"}
            ${results.generalInfo.map(item => ` <li class=""> <a href="${item.permalink}">${item.title} </a>
        ...
        <div class="one-third" >
          ...
          ${results.professors.length ? '<ul class="professor-cards">' : "<p>No Professors Information that matches your searches.</p>"}
          ...
          ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No Professors Information that matches your searches. <a href="${universityData.root_url}/programs" >View all Professors</a> </p>`}
          ${results.programs.map(item => `<li class=""> <a href="${item.permalink}">${item.title} </a></li>`).join("")}
          ${results.programs ? "</ul>" : ""}
        </div>
        <div class="one-third" >
