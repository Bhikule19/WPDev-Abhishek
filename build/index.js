...
function initializeSearch() {
  ...
  function getResults() {
    ...
    const postsURL = universityData.root_url + "/wp-json/wp/v2/posts?search=" + searchValue;
    const pagesURL = universityData.root_url + "/wp-json/wp/v2/pages?search=" + searchValue;
    Promise.all([fetch(postsURL), fetch(pagesURL)]).then(responses => Promise.all(responses.map(response => response.json()))).then(data => {
      ...
      resultsHTML += `<h2 class="search-overlay__section-title">Pages</h2>
          ${pages.length ? '<ul class="link-list min-list">' : "<p>No pages found</p>"}
          ${pages.map(page => `
                <li class="">
